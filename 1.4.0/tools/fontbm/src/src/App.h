...
private:
    ...
    static Glyphs collectGlyphInfo(ft::Font& font, ft::Font* fallbackFont, const std::set<std::uint32_t>& codes);
    static std::uint32_t arrangeGlyphs(Glyphs& glyphs, const Config& config);
    static std::vector<std::string> renderTextures(const Glyphs& glyphs, const Config& config, ft::Font& font, ft::Font* fallbackFont, std::uint32_t pageCount);
